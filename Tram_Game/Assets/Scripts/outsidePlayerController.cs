...
public class outsidePlayerController : MonoBehaviour
{
    ...
    void check_if_picking_up_item_then_proceed()
    {
        if (Input.GetKeyDown(KeyCode.E) && isOnItemStation)
        {
            ...
            {
                Item item = currentItemStation.GetComponent<ItemStationController>().CreateItem();
                inventoryItems.Add(item);
                CanvasManager.GetComponent<outsideChangeUItext>().SpawnItemText(item.itemName);
